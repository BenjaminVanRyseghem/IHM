...
public class JavaSoundAudioDevice extends AudioDeviceBase
{
	...
	{
	    if( (source != null) && (gain >= 0) && (gain <= 1) )
	    {
	        FloatControl volControl = (FloatControl) source.getControl(FloatControl.Type.MASTER_GAIN);
	        float newGain = gain*(volControl.getMaximum()-volControl.getMinimum()) + volControl.getMinimum();
	        volControl.setValue(newGain);
	    }
	}
	
	public float getLineGain()
	{
	    if (source != null)
	    {
	        FloatControl volControl = (FloatControl) source.getControl(FloatControl.Type.MASTER_GAIN);
	        float gain = (volControl.getValue() - volControl.getMinimum()) / (volControl.getMaximum()-volControl.getMinimum());
	        return gain;
	    }
	    return 0;
	}
